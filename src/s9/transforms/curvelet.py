...
class FastCurveletTransform2D(NonLearnableProcessorBase):
    """
...
class InverseFastCurveletTransform2D(NonLearnableProcessorBase):
    """
    ...
    def transform(self, z: torch.Tensor) -> torch.Tensor:
        ...
        z_reshaped = z.view(B, C, num_masks, H, W)
        
        # 역산을 위한 누산기 텐서
        X_f_recon = torch.zeros((B, C, H, W), dtype=torch.complex64, device=z.device)
