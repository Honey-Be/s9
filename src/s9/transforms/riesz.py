...
class RieszTransform(NonLearnableProcessorBase):
    """
    ...
    def _get_multipliers(self, spatial_shape: tuple[int, ...], device: torch.device) -> torch.Tensor:
        ...
        # N차원 주파수 그리드 생성
        grids = [torch.fft.fftfreq(s, device=device) for s in spatial_shape]
    ...
    def transform(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for d in range(self.D):
            ...
            R_d_f = X_f * M_d
            R_d = torch.fft.ifftn(R_d_f, dim=dims).real
...
class InverseRieszTransform(NonLearnableProcessorBase):
    """
    ...
    def transform(self, z: torch.Tensor) -> torch.Tensor:
        ...
        z_reshaped = z.view(B, C, self.D, *spatial_shape)
